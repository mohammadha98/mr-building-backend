...
export class MessengerChannelsService {
  ...
  async createChannel(body: CreateChannelDto) {
    try {
      ...
      // move channel avatar into origin directory
      if (body.avatar) {
        await this.uploadService.moveFile(
          body.avatar,
          "temp/files",
          `messenger/channels/${key}/avatar`
        );
      }
  ...
  // joinChannel
  async joinChannel(body: JoinChannelDto) {
    try {
  ...
  // validateChannelLink
  async validateChannelLink(body: UpdateChannelTypeDto) {
    try {
      const client = await this.prismaService.client.findFirst({
        where: { id: Number(body.client_id) },
      });
  ...
  async ChangeMemberRoleToAdminChannel(body: ChangeMemberRoleToAdminChannel) {
    try {
  ...
  async deleteMessage(message_ids: [number], type: string, room: string) {
    ...
    await Promise.all(
      message_ids.map(async (message_id) => {
        const messageInfo =
          await this.prismaService.messengerChannelsMessages.findFirst({
            where: { id: message_id },
          });
        ...
        if (messageInfo) {
          ...
          if (messageInfo.type !== "text") {
            ...
            this.uploadService.removeFile(
              filename,
              `uploader/${UploaderSources.messenger_channel}/${messageInfo.key}/`
            );
          }
    ...
    if (last_message) {
      last_message.client_info = await this.getClientInfo(
        last_message.owner_id
      );
    } else {
    ...
    try {
      ...
      const presentedChannelsData = await Promise.all(
        channels.map(async (item) => {
          ...
          let message = data.messages[0] as any;
      ...
      const presentedChannels = await this.getChannelsInfo(
        presentedChannelsData
      );
      return {
        status: 200,
  ...
  // requestToOfficialChannel
  async requestToOfficialChannel(body: RequestVerifyChannelDto) {
    try {
      const client = await this.prismaService.client.findFirst({
        where: { id: Number(body.client_id) },
      });
  ...
  // Get my channels
  async getMyChannels(body: GetChannelsDto) {
    try {
  ...
  private async generateKey() {
    const key = "MSGCH" + randomBytes(12).toString("hex").toUpperCase();
  ...
  public async changeStatusForChannel(owner_id: number, status: string) {
    await this.prismaService.messengerChannels.updateMany({
  ...
  public async findChannelByClientId(
    owner_id: number,
    clientId: number,
    tag: string = "normal"
  ) {
    ...
    if (
      ...
      message = channelInfo.messages[0] as any;
      const client_info = await this.getClientInfo(message.owner_id);
      message.client_info = client_info;
  ...
  public async generateUsernameForChannel(channel_id: number) {
    ...
    const username = "CHP" + randomBytes(12).toString("hex").toUpperCase();
    const isDuplicateUsername =
      await this.prismaService.messengerChannels.findFirst({
        where: { username },
      });
  ...
  async saveNewMessage(body: any, client_id: number) {
    try {
      ...
      const client_info = await this.getClientInfo(
        message.messages[0].owner_id
      );
  ...
  async forwardedMessageHanlder(
    messageBody: ForwardMessageIntoChannelMessengerDto
  ) {
    try {
      ...
      for (let index = 0; index < messageBody.messages.length; index++) {
        ...
        if (message.type !== "text") {
          message.content = await this.copyFileForForward(
            message.content,
            messageBody.key,
            UploaderSources.messenger_channel
          );
        }
        ...
        newMessage.client_info = clientInfo as any;
      ...
      const channelTransformer = this.messengerChannelTransformer.transform(
        channelInfo,
