...
export class MarketplaceCategoriesService {
  ...
  public async findActives(pagination: PaginationDto | null, params: any) {
    let  page, per_page, skip

    if (pagination){
      page = PaginationSolver(pagination as any).page
      per_page = PaginationSolver(pagination as any).per_page
      skip = PaginationSolver(pagination as any).skip
    }
