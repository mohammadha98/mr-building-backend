...
import { UpdateWebinarDto } from "src/modules/v2/webinar/app/dto/update-webinar.dto";
...
export default class AlocomWebinarProvider {
  ...
  public async updateUser(userInfo: any) {
    return await this.login().then(async (data) => {
      return await this.webService
        .patch(
          `${this.endpoint}/api/v2/agents/users/${userInfo.agentUser}`,
          userInfo,
          {
            headers: {
              Authorization: this.token,
  ...
  public async addUsersToEvent(event_id: number, users: any) {
    ...
    return await this.webService
      .post(
        `${this.endpoint}/api/v2/agents/events/${event_id}/add-users`,
        users,
        {
          headers: {
            Authorization: this.token,
  ...
  public async updateWebinar(werbinarInfo: UpdateWebinarDto) {
    ...
    return await this.webService
      .patch(
        `${this.endpoint}/api/v2/agents/events/${werbinarInfo.webinar_provider_id}`,
        data,
        {
          headers: { Authorization: this.token },
        }
