...
class AlocomWebinarProvider {
    ...
    async updateUser(userInfo) {
        return await this.login().then(async (data) => {
            return await this.webService
                .patch(`${this.endpoint}/api/v1/agents/users/${userInfo.agentUser}`, userInfo, {
                headers: {
                    Authorization: this.token,
                },
            })
    ...
    async addUsersToEvent(event_id, users) {
        ...
        await this.login();
        return await this.webService
            .post(`${this.endpoint}/api/v1/agents/events/${event_id}/add-users`, users, {
            headers: {
                Authorization: this.token,
            },
        })
    ...
    async updateWebinar(werbinarInfo) {
        ...
        return await this.webService
            .patch(`${this.endpoint}/api/v1/agents/events/${werbinarInfo.webinar_provider_id}`, data, {
            headers: { Authorization: this.token },
        })
            .then(async (data) => {
            if (data.status === 200) {
